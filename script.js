...
// Timetable functionality - FIXED STATISTICS
function initializeTimetable() {
    const timetableForm = document.getElementById('timetable-form');
    ...
    // Initialize empty courses if not exists - COMPLETELY CLEAN
    if (!localStorage.getItem('lifesphere_courses')) {
        localStorage.setItem('lifesphere_courses', JSON.stringify([]));
    } else {
        // Ensure existing data is valid and not dummy data
        const existingCourses = JSON.parse(localStorage.getItem('lifesphere_courses'));
        if (existingCourses.length > 0 && existingCourses[0].isDummy) {
            // Clear dummy data
            localStorage.setItem('lifesphere_courses', JSON.stringify([]));
        }
    }
    ...
    // Load existing courses on page load - UPDATE STATISTICS
...
function updateTimetableStats() {
    ...
    console.log('Updating timetable stats with courses:', courses); // Debug log
    
    // Total classes - FIXED: Show actual count
    if (totalClassesElement) {
        totalClassesElement.textContent = courses.length;
    }
    
    // Weekly hours - FIXED: Calculate properly
    ...
    // Today's classes - FIXED: Calculate properly
    const today = new Date().toLocaleDateString('en-US', { weekday: 'long' }).toLowerCase();
    const todayCourses = courses.filter(course => course.day === today);
...
// Add this function to completely reset timetable data
function resetTimetableData() {
    localStorage.setItem('lifesphere_courses', JSON.stringify([]));
    updateTimetableStats();
    updateTimetableDisplay();
    showNotification('Timetable Reset', 'All timetable data has been cleared.');
}

// Add this to your main initialization to ensure clean start
function initializeApp() {
    initializeNotifications();
    initializeTabNavigation();
    initializeSubtabNavigation();
    ...
    // Ensure timetable starts completely clean
    ensureCleanTimetable();
}

function ensureCleanTimetable() {
    // Force reset timetable data to ensure no dummy data
    if (localStorage.getItem('lifesphere_courses')) {
        const courses = JSON.parse(localStorage.getItem('lifesphere_courses'));
        // If there are any courses, check if they're dummy data
        if (courses.length > 0) {
            // Check for common dummy data patterns
            const hasDummyData = courses.some(course => 
                course.name && (
                    course.name.includes('Sample') ||
                    course.name.includes('Example') ||
                    course.name.includes('Dummy') ||
                    course.isDummy === true
                )
            );
            
            if (hasDummyData) {
                console.log('Found dummy data, clearing timetable...');
                localStorage.setItem('lifesphere_courses', JSON.stringify([]));
            }
        }
    } else {
    ...
    // Update statistics to reflect actual state
    updateTimetableStats();
}

// Make reset function available globally for testing
window.resetTimetableData = resetTimetableData;
